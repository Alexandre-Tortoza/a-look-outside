...
def processar_datasets_balanceados(
    nome_dataset: str = "ambos",
    tecnicas_balanceamento: List[str] = None,
    caminho_datasets: Path = None,
) -> None:
    """
    Processar datasets com técnicas de balanceamento.

    Args:
        nome_dataset: 'sdss', 'decals' ou 'ambos'
        tecnicas_balanceamento: Lista de técnicas a aplicar
        caminho_datasets: Caminho para datasets
    """
    if tecnicas_balanceamento is None:
        tecnicas_balanceamento = ["adasyn", "smote", "undersampling", "oversampling", "estratificacao", "hibrido"]

    logger.info(f"\n{'=' * 80}")
    logger.info("PROCESSANDO DATASETS COM BALANCEAMENTO")
    logger.info(f"{'=' * 80}")

    pipeline = PipelineBalanceamento(
        caminho_datasets=caminho_datasets,
        semente=42,
    )

    logger.info(f"Balanceadores disponíveis: {pipeline.listar_balanceadores()}")

    try:
        pipeline.executar(
            nome_dataset=nome_dataset,
            balanceadores=tecnicas_balanceamento,
            salvar=True,
        )
    except Exception as e:
        logger.error(f"Erro ao processar datasets: {e}")
        raise
...
def main():
    ...
    # Comando: balanceamento
    parser_bal = subparsers.add_parser(
        "balanceamento",
        help="Processar datasets com técnicas de balanceamento",
    )
    parser_bal.add_argument(
        "--dataset",
        choices=["sdss", "decals", "ambos"],
        default="ambos",
        help="Dataset a processar",
    )
    parser_bal.add_argument(
        "--tecnicas",
        nargs="+",
        default=None,
        choices=["adasyn", "smote", "undersampling", "oversampling", "estratificacao", "hibrido"],
        help="Técnicas de balanceamento a aplicar",
    )
    parser_bal.add_argument(
        "--data-dir",
        type=Path,
    ...
    # Comando: benchmark (padrão)
    parser_bench = subparsers.add_parser(
        "benchmark",
        help="Executar benchmark de modelos",
    )
    ...
    # Se nenhum comando for especificado, usar benchmark como padrão
    if args.comando is None:
        args = parser.parse_args(["benchmark"])

    # Processar comando de balanceamento
    if args.comando == "balanceamento":
        processar_datasets_balanceados(
            nome_dataset=args.dataset,
            tecnicas_balanceamento=args.tecnicas,
            caminho_datasets=args.data_dir,
        )
        return

    # Processamento padrão: benchmark
