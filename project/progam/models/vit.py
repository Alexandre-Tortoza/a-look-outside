...
class ViTLight(GalaxyClassifier):
    ...
    def __init__(self, modo_treino: str = "scratch"):
        """
        Args:
            modo_treino: "scratch" (treina do zero) ou "finetune" (usa timm pretrained)
        """
        super().__init__()
    ...
    def build(self, num_classes: int, img_size: int) -> nn.Module:
        ...
        if self.modo_treino == "scratch":
        ...
        elif self.modo_treino == "finetune":
            try:
                import timm
            except ImportError:
                raise ImportError("timm é necessário para fine-tuning. Instale com: pip install timm")
            ...
            # Congela backbone, apenas cabeça é treinável
            ...
            raise ValueError(f"Modo inválido: {self.modo_treino}. Use 'scratch' ou 'finetune'.")
