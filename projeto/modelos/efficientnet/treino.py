"""Pipeline de treino do EfficientNet."""
...
def treinar(config_override: Optional[dict] = None) -> HistoricoTreino:
    params = {
        ...
        "seed": cfg.SEED, "pretrained": cfg.PRETRAINED, "variante_backbone": cfg.VARIANTE_BACKBONE,
        "salvar_pesos": cfg.SALVAR_PESOS, "num_workers": cfg.NUM_WORKERS,
        "paciencia_early_stop": cfg.PACIENCIA_EARLY_STOP, "scheduler_ativo": cfg.SCHEDULER_ATIVO,
        "peso_decay": cfg.PESO_DECAY, "nome_experimento": cfg.NOME_EXPERIMENTO,
    }
    if config_override:
        params.update(config_override)

    log = obter_logger(__name__, arquivo_log=Path("docs/efficientnet/treino.log"))
    ...
    rede = EfficientNetGalaxy(
        variante_backbone=params["variante_backbone"], pretrained=params["pretrained"]
